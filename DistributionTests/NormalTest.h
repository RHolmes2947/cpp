...
public:
    NormalTest(int numSamples) : Test<T>(numSamples) {
        ...
        std::mt19937 gen(rd());
        std::normal_distribution<T> dis(0.0, 10.0); // Mean = 0, Stddev = 10
    ...
    std::vector<int> getHistogram() override {
        ...
        for (const T& val : this->data) {
            ...
            histogram[bucket * bucketSize + static_cast<int>(minVal)]++;
        }

        std::vector<int> histVec(20, 0);
        for (const auto& pair : histogram) {
            int bucket = static_cast<int>((pair.first - static_cast<int>(minVal)) / bucketSize);
